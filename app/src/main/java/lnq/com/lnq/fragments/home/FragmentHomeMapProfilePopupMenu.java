...
public class FragmentHomeMapProfilePopupMenu extends Fragment implements CompoundButton.OnCheckedChangeListener {

    //    Android fields....
    private FragmentHomeMapProfilePopupMenuBinding popUpMenuBinding;
    private PopupMenuClickHandler clickHandler;
    ...
    public FragmentHomeMapProfilePopupMenu() {
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        popUpMenuBinding = DataBindingUtil.inflate(inflater, R.layout.fragment_home_map_profile_popup_menu, container, false);
        return popUpMenuBinding.getRoot();
    }
        ...
        if (getArguments() != null) {
            ...
            if (userConnectionStatus != null) {
                if (isUserFavorite.equals(Constants.FAVORITE)) {
                    popUpMenuBinding.textViewUnFavorite.setText(getResources().getString(R.string.un_favorite));
                } else {
                    popUpMenuBinding.textViewUnFavorite.setText(getResources().getString(R.string.favorite));
                }
                if (userConnectionStatus.equals(Constants.CONNECTED)) {
                    popUpMenuBinding.textViewUnLnq.setVisibility(View.VISIBLE);
                    popUpMenuBinding.viewDividerFour.setVisibility(View.VISIBLE);
                } else {
                    popUpMenuBinding.textViewUnLnq.setVisibility(View.GONE);
                    popUpMenuBinding.viewDividerFour.setVisibility(View.INVISIBLE);
                }
                if (isLocationHidden.equalsIgnoreCase(Constants.HIDDEN)) {
                    popUpMenuBinding.switchCompatLocationHidden.setChecked(true);
                } else {
                    popUpMenuBinding.switchCompatLocationHidden.setChecked(false);
                }
                popUpMenuBinding.switchCompatLocationHidden.setOnCheckedChangeListener(this);
            }
        }

//        Setting click handler for data binding....
        clickHandler = new PopupMenuClickHandler();
        popUpMenuBinding.setClickHandler(clickHandler);

//        All event listeners....
        popUpMenuBinding.switchCompatFullyBlock.setOnCheckedChangeListener(this);

        popUpMenuBinding.mRoot.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View view, MotionEvent motionEvent) {
                ValidUtils.hideKeyboardFromFragment(getActivity(), popUpMenuBinding.mRoot);
                return false;
            }
        });
    ...
    private void setCustomFont() {
        ...
        fontUtils.setTextViewSemiBold(popUpMenuBinding.textViewSendMessage);
        fontUtils.setTextViewRegularFont(popUpMenuBinding.textViewLocationHidden);
        fontUtils.setTextViewSemiBold(popUpMenuBinding.textViewUnFavorite);
        fontUtils.setTextViewRegularFont(popUpMenuBinding.textViewUnLnq);
        fontUtils.setTextViewRegularFont(popUpMenuBinding.textViewFullyBlocks);
    }
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        if (buttonView == popUpMenuBinding.switchCompatFullyBlock) {
            if (isChecked) {
        ...
        } else if (buttonView == popUpMenuBinding.switchCompatLocationHidden) {
            if (isChecked) {
...
//        callShowHideLocation = Api.WEB_SERVICE.hideShowLocation(EndpointKeys.X_API_KEY, DateUtils.getCurrentTime(), Credentials.basic(LnqApplication.getInstance().sharedPreferences.getString(EndpointKeys.USER_EMAIL, ""), LnqApplication.getInstance().sharedPreferences.getString(EndpointKeys.USER_PASSWORD, "")), LnqApplication.getInstance().sharedPreferences.getString("id", ""), userId, status);
    ...
    public class PopupMenuClickHandler {
