...
public class BiometricManagerPrint extends BiometricManagerV23 {


    protected CancellationSignal mCancellationSignal = new CancellationSignal();

    @RequiresApi(api = Build.VERSION_CODES.M)
    protected BiometricManagerPrint(final BiometricBuilder biometricBuilder) {
    ...
    @RequiresApi(api = Build.VERSION_CODES.M)
    public void authenticate(@NonNull final BiometricCallbackPrint biometricCallback) {
        ...
        if(!BiometricUtilsPrint.isSdkVersionSupported()) {
            biometricCallback.onSdkVersionNotSupported();
            return;
        }

        if(!BiometricUtilsPrint.isPermissionGranted(context)) {
            biometricCallback.onBiometricAuthenticationPermissionNotGranted();
            return;
        }

        if(!BiometricUtilsPrint.isHardwareSupported(context)) {
            biometricCallback.onBiometricAuthenticationNotSupported();
            return;
        }

        if(!BiometricUtilsPrint.isFingerprintAvailable(context)) {
    ...
    public void cancelAuthentication(){
        if(BiometricUtilsPrint.isBiometricPromptEnabled()) {
    ...
    @RequiresApi(api = Build.VERSION_CODES.M)
    private void displayBiometricDialog(BiometricCallbackPrint biometricCallback) {
//        if(BiometricUtilsPrint.isBiometricPromptEnabled()) {
...
//        }
    ...
    private void displayBiometricPrompt(final BiometricCallbackPrint biometricCallback) {
    ...
    public static class BiometricBuilder {
        ...
        public BiometricManagerPrint.BiometricBuilder setTitle(@NonNull final String title) {
            this.title = title;
            return this;
        }

        public BiometricManagerPrint.BiometricBuilder setSubtitle(@NonNull final String subtitle) {
            this.subtitle = subtitle;
            return this;
        }

        public BiometricManagerPrint.BiometricBuilder setDescription(@NonNull final String description) {
            this.description = description;
            return this;
        }


        public BiometricManagerPrint.BiometricBuilder setNegativeButtonText(@NonNull final String negativeButtonText) {
            this.negativeButtonText = negativeButtonText;
            return this;
        }

        @RequiresApi(api = Build.VERSION_CODES.M)
        public BiometricManagerPrint build() {
            return new BiometricManagerPrint(this);
        }
    }
}
