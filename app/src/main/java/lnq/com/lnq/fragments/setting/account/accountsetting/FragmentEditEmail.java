...
import lnq.com.lnq.databinding.FragmentEditEmailBinding;
...
public class FragmentEditEmail extends Fragment implements View.OnClickListener {

    //    Android fields....
    private FragmentEditEmailBinding mBind;

    //    Api fields....
    private Call<RegisterLoginMainObject> callUpdateEmail;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mBind = DataBindingUtil.inflate(inflater, R.layout.fragment_edit_email, container, false);
    ...
    private void init() {
        ...
        mBind.mBtnUpdateEmail.setOnClickListener(this);
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.mBtnGoBack:
                ...
                break;
            case R.id.mBtnUpdateEmail:
                ValidUtils.hideKeyboardFromFragment(getActivity(), mBind.getRoot());
                if (!mBind.editTextNewEmail.getText().toString().isEmpty()) {
                    if (!mBind.editTextNewPassWord.getText().toString().isEmpty()) {
                        if (mBind.editTextNewPassWord.getText().toString().equals(LnqApplication.getInstance().sharedPreferences.getString("user_pass", ""))) {
                            reqUpdateEmail(mBind.editTextNewEmail.getText().toString(), mBind.editTextNewPassWord.getText().toString());
                        } else {
                    ...
                    ((MainActivity) getActivity()).showMessageDialog("error", "Enter New Email");
                }
                break;
        }
    }

    private void reqUpdateEmail(final String newEmail, String userPass) {
        ((MainActivity) getActivity()).progressDialog.show();
        callUpdateEmail = Api.WEB_SERVICE.updateEmail(EndpointKeys.X_API_KEY, Credentials.basic(LnqApplication.getInstance().sharedPreferences.getString(EndpointKeys.USER_EMAIL, ""), LnqApplication.getInstance().sharedPreferences.getString(EndpointKeys.USER_PASSWORD, "")), LnqApplication.getInstance().sharedPreferences.getString(EndpointKeys.ID, ""), LnqApplication.getInstance().sharedPreferences.getString(EndpointKeys.USER_EMAIL, ""), newEmail, userPass);
//        callUpdateEmail = Api.WEB_SERVICE.updateEmail(EndpointKeys.X_API_KEY, DateUtils.getCurrentTime(), Credentials.basic(LnqApplication.getInstance().sharedPreferences.getString(EndpointKeys.USER_EMAIL, ""), LnqApplication.getInstance().sharedPreferences.getString(EndpointKeys.USER_PASSWORD, "")), LnqApplication.getInstance().sharedPreferences.getString(EndpointKeys.ID, ""), LnqApplication.getInstance().sharedPreferences.getString(EndpointKeys.USER_EMAIL, ""), newEmail, userPass);
        callUpdateEmail.enqueue(new Callback<RegisterLoginMainObject>() {
            @Override
            public void onResponse(Call<RegisterLoginMainObject> call, Response<RegisterLoginMainObject> response) {
                ...
                if (response != null && response.isSuccessful()) {
                    switch (response.body().getStatus()) {
                        case 1:
                            ((MainActivity) getActivity()).progressDialog.dismiss();
                            LnqApplication.getInstance().editor.putString(EndpointKeys.USER_EMAIL, newEmail).apply();
                            EventBus.getDefault().post(new EventBusUserSession("email_updated"));
                            EventBus.getDefault().post(new EventBusPrimaryEmailProfile());
