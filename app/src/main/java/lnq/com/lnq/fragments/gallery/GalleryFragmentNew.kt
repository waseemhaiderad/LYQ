...
class GalleryFragmentNew : BottomSheetDialogFragment(), View.OnClickListener, GalleryAdopter.onClickAdapterInterface {
...
//            delay(5000)
            ...
            if (result) {
                ...
                listGallery.reverse()
                adopterGallery = GalleryAdopter(listGallery, this@GalleryFragmentNew)
...
//                        binding.mTvTitle.text = "0"
    ...
    suspend fun loadGalleryData(): Boolean = withContext(Dispatchers.IO) {
        ...
        if (cur != null && cur.moveToFirst()) {
            ...
            val mCurAlbum = requireActivity().contentResolver.query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, mAlbums, null, null, null)
...
//                    mArAlbums[mCurAlbum.position] = mCurAlbum.getString(mCurAlbum.getColumnIndex(MediaStore.Images.ImageColumns.BUCKET_DISPLAY_NAME))
    ...
    companion object {
        ...
        @JvmStatic
        fun newInstance(retrieveString: String?): GalleryFragmentNew? {
            val f = GalleryFragmentNew()
            val args = Bundle()
